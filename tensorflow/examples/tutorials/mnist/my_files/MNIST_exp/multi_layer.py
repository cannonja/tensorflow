...
## Read and unpack images, then launch interactive session
...
## Conv stride = 1, conv pad = 0
## Max pooling = 2x2 blocks
def conv2d(x, W):
...
############# Define input and target label placeholders #############

x = tf.placeholder(tf.float32, shape=[None, 784])
y_ = tf.placeholder(tf.float32, shape=[None, 10])


############## Implement first convolutional layer ####################

'''
Will consist of convolution followed by max pooling
Convolution has 32 feature maps over 5x5 patches
...
################# Implement second convolutional layer #######################

## Second layer has 64 featur maps for each 5x5 patch
...
off during testing. TensorFlow's tf.nn.dropout op automatically handles scaling
neuron outputs in addition to masking them, so dropout just works without
...
############## Final softmax layer (readout/output layer) ##################
...
with the more sophisticated ADAM optimizer; we will include the additional
parameter keep_prob in feed_dict to control the dropout rate; and we will add
logging to every 100th iteration in the training process.
'''
